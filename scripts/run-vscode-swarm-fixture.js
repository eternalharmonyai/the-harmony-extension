...
const tempBase = fs.mkdtempSync(path.join(os.tmpdir(), 'harmony-vscode-fixture-host-'));
...
(async () => {
    try {
        await runTests({
            ...
            extensionTestsPath: path.join(root, 'test', 'vscode-swarm-commit-fixture.js'),
            launchArgs: [
                workspace,
                '--disable-extensions',
        ...
        try {
            ...
            // Best-effort cleanup only; the fixture report records its own disposable repo cleanup.
        }
    }
})().catch((error) => {
